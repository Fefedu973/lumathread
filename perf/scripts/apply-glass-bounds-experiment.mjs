...
const requestedMode =
  ...
  process.env.LUMATHREAD_GLASS_BOUNDS_EXPERIMENT ??
  process.argv[2] ??
  "blit";
if (requestedMode !== "scissor" && requestedMode !== "blit") {
  throw new Error(
    `Unknown glass bounds experiment ${JSON.stringify(requestedMode)}. Expected scissor or blit.`,
  );
}
...
const stableBlitCondition =
  requestedMode === "blit"
    ? `exactGl !== null &&
      introProgress >= 0.999999 &&
      sceneOpacity >= 0.999999`
    : `exactGl !== null &&
      resourceState.canvasWidth < 0 &&
      introProgress >= 0.999999 &&
      sceneOpacity >= 0.999999`;
await replaceOnce(
  "src/runtime/glass-terrain-renderer.ts",
...
console.log(`Applied bounded glass ${requestedMode} experiment.`);
